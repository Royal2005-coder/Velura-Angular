...
2. GitHub: được mời vào https://github.com/Royal2005-coder/Velura-Angular (Write). Không push `main` / `develop`.
...
3. Commit + **title PR** bắt đầu `KAN-<n>`.
4. PR **vào develop**. CI: validate + `test-api` + `test-angular` + build + `note-pr`.
5. Không merge CI đỏ. Không feature → `main`. Không `[skip ci]`.
6. Production chỉ sau PR `develop` → `main`. `develop` không deploy.

Luồng: [HOW-IT-WORKS.md](./HOW-IT-WORKS.md). UI: [ANGULAR-STANDARDS.md](./ANGULAR-STANDARDS.md).

## Practice (một lần / người)

Mỗi người một Task Jira (không dùng chung KAN-13/KAN-14). Draft PR vào `develop`, **không merge**.
...
Title: `KAN-n Practice for <tên>`. Mark as draft. Đợi CI xanh + comment Trace. Close PR (không Merge). Jira Close: `Practice complete; PR not merged.`
